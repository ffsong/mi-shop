...
class CreateOrdersTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('orders', function (Blueprint $table) {
            ...
            $table->string('refund_status')->default(\App\Order::REFUND_STATUS_PENDING)->comment('退款状态');
            $table->string('refund_no')->nullable()->unique()->comment('退款订单号');
            $table->tinyInteger('closed')->default(0)->comment('订单是否关闭');
            $table->tinyInteger('reviewed')->default(0)->comment('订单是否已评价');
            $table->string('ship_status')->default(\App\Order::SHIP_STATUS_PENDING)->comment('物流状态');
